	...
	public function getTimeline($username, $displayHtml = true, $page = 1) {
		...
		if($displayHtml) {
			...
			while($rw = mysqli_fetch_row($rs)) {
				...
				$html .= "<tr><td class='headimg'><img src='https://sdn.geekzu.org/avatar/" . md5($this->getUserInfo($rw[2])['email']) . "?s=64'</td><td class='thread'><p><small>{$rw[2]} 发表于" . $this->getDateFormat($rw[3]) . "&nbsp;&nbsp;<a href='?s=msg&id={$rw[0]}' target='_blank'><i class='fa fa-external-link'></i></a>" . $pstatus . str_replace("{id}", $rw[0], $sdelete) . "</small></p>";
